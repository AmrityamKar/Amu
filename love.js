(function(window){
    ...
    Seed.prototype = {
        ...
        */
        drawHeart: function() {
            var s = this, ctx = s.tree.ctx, heart = s.heart;
            var point = heart.point, color = heart.color, 
                scale = heart.scale;
            ...
            // Draw the balloon body (ellipse)
            ctx.beginPath();
            ctx.ellipse(2, -21, 15, 20, 0, 0, 2 * Math.PI); // Balloon body
            ctx.fill();
            ctx.closePath();
        
            // Draw the balloon knot (small circle at the bottom)
            ctx.beginPath();
            ctx.arc(2, 0, 3, 0, 2 * Math.PI); // Balloon knot
            ctx.fill();
            ctx.closePath();
        
            // Draw the balloon string (line)
            ctx.beginPath();
            ctx.moveTo(0, 0); // Start of the string
            ctx.lineTo(0, 0); // End of the string
            ctx.strokeStyle = "#000000"; // Black color for the string
        ...
        drawText: function() {
            ...
            ctx.fillText("Hey !! ", 30, -5);
            ctx.fillText("Tap on the baloon to burst!", 28, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
            var point = cirle.point, scale = cirle.scale, radius = 26;
            var w = (radius * scale); // Width to clear
            var h = (43 * scale); // Height to clear
        
            // Adjust the clearRect area to cover the line below the text
            ctx.clearRect(point.x - w, point.y - h, 4 * w + 120, 4 * h + 120); // Added 20 pixels to cover the line
        },
        hover: function(x, y) {
   ...
   Bloom.prototype = {
        ...
        draw: function() {
            ...
            // Draw a circle instead of a heart
            ctx.beginPath();
            ctx.arc(0, 0, 10, 0, 2 * Math.PI); // Radius of 10, adjust as needed
            ctx.closePath();
            ctx.fill();
